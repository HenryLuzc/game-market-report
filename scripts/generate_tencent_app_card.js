#!/usr/bin/env node
/**
 * Generate Feishu interactive card JSON for Tencent APP game data.
 * Usage: node generate_tencent_app_card.js <input.json> [output.json]
 */
const fs = require('fs');
...
function buildAnalysis(games) {
  ...
  const headTailRatio = games.length ? Math.floor(games[0].daily_cost / lastCost) : 1;
...
function buildCard(data) {
  ...
  const title = data.title || `${data.date_range || ''} 腾讯手游消耗排名分析`;
...
if (args.length < 1) {
  console.error('Usage: node generate_tencent_app_card.js <input.json> [output.json]');
  process.exit(1);
}
