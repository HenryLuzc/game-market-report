#!/usr/bin/env node
/**
 * Generate Feishu interactive card JSON from structured game data.
...
function calcSummary(games, totalDailyCost) {
  ...
  // Use total_daily_cost from spreadsheet header if available (represents the real platform total),
...
function buildAnalysis(games) {
  ...
  for (const g of games) {
    ...
    if (g.client) {
      clientCost[g.client] = (clientCost[g.client] || 0) + g.daily_cost;
    }
  }
  ...
  // 最大客户
  const sortedClients = Object.entries(clientCost).sort((a, b) => b[1] - a[1]);
  const topClientName = sortedClients[0]?.[0];
  const topClientVal = sortedClients[0]?.[1] || 0;
...
function buildCard(data) {
  ...
  return {
    ...
    elements: [
      { tag: 'markdown', content: `共 **${total}** 款游戏  |  日均总消耗 **${totalCost.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} 万元**` },
      { tag: 'hr' },
