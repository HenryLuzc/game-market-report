...
function buildAnalysis(games, summary) {
  ...
  const top10Pct = totalCost > 0 ? +(top10Cost / totalCost * 100).toFixed(1) : 0;
  ...
  lines.push(`**${escapeMd(topTagName)}类主导市场**：占总消耗 ${topTagPct}%（日均 ${Math.round(topTagVal).toLocaleString()} 万），共 ${topTagCnt} 款`);
